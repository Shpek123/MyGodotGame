...
@onready var fps_component: FPSComponent = $FPSComponent
...
func _physics_process(delta: float) -> void:
	...
	movement_component.direction = (fps_component.pivot.transform.basis * Vector3(input_component.move_dir.x, 0, input_component.move_dir.y)).normalized()
